import React, {Component} from 'react';
import { AppRegistry, Image, StyleSheet, Text, TextInput, Alert, Button, ScrollView, View } from 'react-native';
import {createStackNavigator,} from 'react-navigation';

class Historique extends React.Component {
  static navigationOptions = {
    title: 'Accueil',
    headerTitle: (
    <View style={{flexDirection: 'row', justifyContent: 'space-between', margin: 20}}>
      <Text style={{fontSize: 25}}>Historique</Text>
      <Image source={require('../../image/logo2.png')} style={{height: 50, marginLeft: 125}}/>
    </View>
    ),
  };
  render(){
    let pic1 = {
        uri: 'https://www.orange.sn/2/sites/particuliers/1/441/orange-money-senegal-500x300.jpg'
      };
      let pic2 = {
        uri: 'data:image/png;base64,'
      };
      let pic3 = {
        uri: 'https://cdn6.aptoide.com/imgs/5/2/6/5268ec541aed55d2b8d733be35ff5862_icon.png?w=240'
      };
      let pic4 = {
        uri: 'https://www.tigo.sn/images/customer-care/tigo_cash.PNG'
      };
      let pic5 = {
        uri: 'https://www.rue23.net/wp-content/uploads/2018/03/E-Money-by-Expresso-Senegal-740x431.jpg'
      };
    return(
      <ScrollView style={{backgroundColor: '#FFF'}}>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic1} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic1} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic5} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic3} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic5} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic1} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic5} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic2} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic5} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
        <View style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 60, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic4} style={{marginLeft: 10, width: 50, height: 50}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'row', justifyContent: 'space-between'}}>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
              <Text>09 oct 2018</Text>
              <Text>775698715</Text>
              <Text style={{fontSize: 10,}}>N°2548</Text>
            </View>
            <View style={{flex: 1, flexDirection: 'column', justifyContent: 'space-between', alignItems: 'center'}}>
              <Text>22H31</Text>
              <Text>250 fcfa</Text>
            </View>
          </View>
        </View>
      </ScrollView>
    );
  }
}

export default Historique;
