import React, {Component} from 'react';
import { AppRegistry, Image, StyleSheet, Text, TouchableOpacity, TextInput, Alert, Button, ScrollView, View } from 'react-native';
import {createStackNavigator,} from 'react-navigation';

class Wallet extends React.Component {
  static navigationOptions = {
    title: 'Wallet',
  };
  render(){
    let pic1 = {
        uri: 'https://www.orange.sn/2/sites/particuliers/1/441/orange-money-senegal-500x300.jpg'
      };
      let pic2 = {
        uri: 'data:image/png;base64,'
      };
      let pic3 = {
        uri: 'https://cdn6.aptoide.com/imgs/5/2/6/5268ec541aed55d2b8d733be35ff5862_icon.png?w=240'
      };
      let pic4 = {
        uri: 'https://www.tigo.sn/images/customer-care/tigo_cash.PNG'
      };
      let pic5 = {
        uri: 'https://www.rue23.net/wp-content/uploads/2018/03/E-Money-by-Expresso-Senegal-740x431.jpg'
      };
      const { navigate } = this.props.navigation;
    return(
      <ScrollView style={{backgroundColor: '#FFF'}}>
        <View>
      <TouchableOpacity
        onPress={ () => navigate('walletom') }
        style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 100, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}
        >
          <Image source={pic1} style={{marginLeft: 10, width: 80, height: 80}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
            <Text>Le Solde de Votre Compte:</Text>
            <Text>25.684 fcfa</Text>
          </View>
            </TouchableOpacity>
        </View>
        <View>
          <TouchableOpacity
            onPress={ () => navigate('walletwari') }
            style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 100, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic2} style={{marginLeft: 10, width: 80, height: 80}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
            <Text>Le Solde de Votre Compte:</Text>
            <Text>1.000 fcfa</Text>
          </View>
        </TouchableOpacity>
        </View>
        <View>
          <TouchableOpacity
            onPress={ () => navigate('walletjoni') }
            style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 100, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic3} style={{marginLeft: 10, width: 80, height: 80}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
            <Text>Le Solde de Votre Compte:</Text>
            <Text>0 fcfa</Text>
          </View>
        </TouchableOpacity>
        </View>
        <View>
          <TouchableOpacity
            onPress={ () => navigate('wallettigo') }
            style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 100, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic4} style={{marginLeft: 10, width: 80, height: 80}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
            <Text>Le Solde de Votre Compte:</Text>
            <Text>500 fcfa</Text>
          </View>
        </TouchableOpacity>
        </View>
        <View>
          <TouchableOpacity
            onPress={ () => navigate('wallete') }
            style={{marginTop: 10, marginLeft: 5, shadowColor: 'grey', shadowOpacity: 5, shadowRadius: 5, backgroundColor: '#FFF', alignItems: 'center', width: 350, height: 100, borderColor: 'grey', borderStyle: 'solid', borderBottomWidth: 3, borderWidth: 1, borderRadius: 20, flex: 1, flexDirection: 'row', justifyContent: 'space-between'
      }}>
          <Image source={pic5} style={{marginLeft: 10, width: 80, height: 80}}/>
          <View style={{marginLeft: 50, flex: 1, flexDirection: 'column', justifyContent: 'space-between'}}>
            <Text>Le Solde de Votre Compte:</Text>
            <Text>1.658.548fcfa</Text>
          </View>
        </TouchableOpacity>
        </View>
      </ScrollView>
    );
  }
}

export default Wallet;
